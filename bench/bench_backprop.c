...
static float train_xor(int stochastic, int epochs) {
    ...
    tk_optim opt = tk_optim_default(0.5f); opt.stochastic = stochastic;
    float z1[H],h[H],z2[OUT],y[OUT],dy[OUT],dh[H],dW1[H*IN],db1[H],dW2[OUT*H],db2[OUT],loss=0;
    for (int e=0;e<epochs;e++){ loss=0; for(int s=0;s<4;s++){ tk_scalar_t xq[IN],hq[H];
        for(int i=0;i<IN;i++)xq[i]=TK_FROM_FLOAT(X[s][i]);
        tk_linear_forward(W1,xq,b1,z1,H,IN,TK_ACT_IDENTITY);
        for(int i=0;i<H;i++)h[i]=z1[i]; tk_activate(h,H,TK_ACT_TANH);
        for(int i=0;i<H;i++)hq[i]=TK_FROM_FLOAT(h[i]);
        tk_linear_forward(W2,hq,b2,z2,OUT,H,TK_ACT_IDENTITY);
        y[0]=z2[0]; tk_activate(y,OUT,TK_ACT_SIGMOID);
        ...
        tk_sgd_step(W2,dW2,OUT*H,&opt,&rng); tk_sgd_step(b2,db2,OUT,&opt,&rng);
        tk_sgd_step(W1,dW1,H*IN,&opt,&rng);  tk_sgd_step(b1,db1,H,&opt,&rng);
    }}
    return loss/4.0f;
}
...
int main(void) {
    ...
    for (int i=0;i<OUT;i++){ z[i]=0.1f; dy[i]=0.01f; }

    printf("=== mantissa backprop benchmark  (dtype=%s) ===\n", tk_dtype_name());
    printf("layer %dx%d = %ld params\n", OUT, IN, params);

    tk_linear_backward(W,x,z,dy,dW,db,dx,OUT,IN,TK_ACT_RELU);   /* warm up */
    double t0=now_s();
    for(int r=0;r<REPS;r++){ tk_linear_backward(W,x,z,dy,dW,db,dx,OUT,IN,TK_ACT_RELU); g_sink+=dW[r%params]; }
    double dt=now_s()-t0;
    ...
    /* Stochastic rounding vs round-to-nearest: same run, final XOR loss. */
    ...
    free(W);free(x);free(z);free(dy);free(dW);free(db);free(dx);
    return 0;
}
